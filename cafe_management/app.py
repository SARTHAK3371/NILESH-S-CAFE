...
app.config['SECRET_KEY'] = os.getenv(
    'SECRET_KEY',
    'cafe-secret-key-change-in-production-2024'
)
    ...
    if MenuItem.query.count() == 0:
        ...
        samples = [
            ...
            (
                ...
                80,
                'Coffee',
                'https://images.unsplash.com/photo-1510591509098-f4fdc6d0f0b6?w=400'
            ),
...
def dashboard():
    ...
    preparing = Order.query.filter_by(
        status='preparing'
    ).count()

    available_tables = Table.query.filter_by(
        status='available'
    ).count()
...
def add_menu_item():
    ...
    img = request.form.get(
        ...
        'photo-1495474472287-4d71bcdd2085?w=400'
    )
...
def edit_menu_item(id):
    ...
    item.image_url = request.form.get(
        'image_url'
    )
...
def tables():
    ...
    all_tables = Table.query.order_by(
        Table.number
    ).all()
...
def customers():
    ...
    all_customers = Customer.query.order_by(
